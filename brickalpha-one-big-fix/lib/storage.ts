...
const INV="lih_inventory_v2", WATCH="lih_watch_v1";
export function getInventory():LegoItem[]{if(typeof window==="undefined")return seedInventory;const raw=localStorage.getItem(INV)||localStorage.getItem("lih_inventory_v1");if(!raw){localStorage.setItem(INV,JSON.stringify(seedInventory));return seedInventory}try{return JSON.parse(raw)}catch{return seedInventory}}
